...
function fillFormData(item, fields) {
    fields.forEach(field => {
        ...
        if (field.type === 'multiselect') {
            ...
            if (field.name === 'startsAt' || field.name === 'expiresAt') {
                if (val) {
                    const date = new Date(val);
                    if (!Number.isNaN(date.getTime())) {
                        val = new Date(date.getTime() - (date.getTimezoneOffset() * 60000)).toISOString().slice(0, 16);
                    }
                }
            }
            if (field.name === 'active') {
                val = item.active === false ? 'false' : 'true';
            }
